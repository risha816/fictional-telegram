...
const CHANNEL_CATEGORIES = [
  ...
  {id:'adult',  name:'18+ ওয়েব সিরিজ',      link:config.channelPortal.adult,   badge:'বোল্ড · থ্রিলার · ক্রাইম ড্রামা',grad:'from-[#3b0764] to-[#6b21a8]', icon:<Flame size={17}/>},
  {id:'natok',  name:'বাংলাদেশি নাটক',       link:config.channelPortal.natok,   badge:'Chorki · Hoichoi · Binge',     grad:'from-[#14532d] to-[#166534]', icon:<Clapperboard size={17}/>},
];
  ...
  const loadTMDB = useCallback(async()=>{
    ...
    try {
      ...
      const [r2,r3,r4,r5,r6,r7,r8,r9] = await Promise.all([
        ...
        // 🔞 18+ Bold/Thriller web series (crime + thriller genre)
        fetch(`${TMDB_BASE}/discover/tv?api_key=${TMDB_KEY}&with_genres=80%2C53&sort_by=popularity.desc&vote_average.gte=7`).then(r=>r.json()),
      ]);
          ...
          {!isSearchActive && !viewAllCategory && (
            <motion.div key="hub" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              ...
              <div className="space-y-7 pb-8">
                ...
                <PosterRow title="🔞 বোল্ড ওয়েব সিরিজ" data={adultSeries}
                  onPosterClick={m=>handlePosterClick(m,config.rows.adultSeries)}
                  onViewAll={()=>setViewAllCategory({title:'বোল্ড ওয়েব সিরিজ', data:adultSeries,  link:config.rows.adultSeries})}/>
                {natoklist.length > 0 && (
