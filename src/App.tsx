...
const AD_COOLDOWN_MS   = 24 * 60 * 60 * 1000; // 24 ঘণ্টা

// 24hr cooldown check
const getAdKey = () => 'cv_ad_last_shown';
const hasSeenAdRecently = (): boolean => {
  try {
    const last = localStorage.getItem(getAdKey());
    if (!last) return false;
    return Date.now() - parseInt(last) < AD_COOLDOWN_MS;
  } catch { return false; }
};
const markAdShown = () => {
  try { localStorage.setItem(getAdKey(), Date.now().toString()); } catch {}
};
...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────
// AdPage — full screen ad before going to channel
// ─────────────────────────────────────────────
const AdPage = memo(({channelUsername, onDone}:{channelUsername:string; onDone:()=>void}) => {
  const [status, setStatus] = React.useState<'loading'|'adsgram'|'monetag'|'done'>('loading');
  const [countdown, setCountdown] = React.useState(5);
  const monetagRef = React.useRef<HTMLDivElement>(null);
  const adsgram = React.useRef<any>(null);

  const goToChannel = React.useCallback(()=>{
    markAdShown();
    const tg = (window as any).Telegram?.WebApp;
    const url = channelUsername.startsWith('http')
      ? channelUsername
      : `https://t.me/${channelUsername}`;
    if(tg) tg.openTelegramLink(url.startsWith('https://t.me') ? url : `https://t.me/${channelUsername}`);
    else window.open(url, '_blank');
    onDone();
  }, [channelUsername, onDone]);

  // Adsgram চেষ্টা
  React.useEffect(()=>{
    const tryAdsgram = async () => {
      try {
        const AdController = await (window as any).Adsgram?.init({ blockId: ADSGRAM_BLOCK_ID });
        adsgram.current = AdController;
        await AdController.show();
        // ad সফলভাবে দেখানো হয়েছে
        markAdShown();
        goToChannel();
      } catch(e: any) {
        // Adsgram fail → Monetag চেষ্টা
        setStatus('monetag');
      }
    };

    // Adsgram SDK load হওয়া পর্যন্ত অপেক্ষা
    if ((window as any).Adsgram) {
      tryAdsgram();
    } else {
      const script = document.createElement('script');
      script.src = 'https://sad.adsgram.ai/js/sad.min.js';
      script.onload = tryAdsgram;
      script.onerror = () => setStatus('monetag');
  ...
  // Monetag fallback
  React.useEffect(()=>{
    if (status !== 'monetag') return;

    // Monetag SDK inject
    const script = document.createElement('script');
    script.src = `//libtl.com/sdk.js`;
    script.setAttribute('data-zone', MONETAG_ZONE_ID);
    script.setAttribute('data-sdk', 'show_9239674');
    document.head.appendChild(script);
    ...
    const t = setInterval(()=>{
      setCountdown(c=>{
        if(c <= 1){ clearInterval(t); goToChannel(); return 0; }
        return c-1;
      });
    }, 1000);
  ...
  return (
    <div className="fixed inset-0 z-[999] flex flex-col items-center justify-center"
      ...
      {/* Top bar */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#A855F7] to-[#4FC3F7]"
        style={{width: status==='monetag' ? `${(5-countdown)/5*100}%` : '30%', transition:'width 1s linear'}}/>

      <div className="flex flex-col items-center gap-6 px-8 text-center">
        {/* Icon */}
        <div className="h-20 w-20 rounded-3xl flex items-center justify-center"
          style={{background:'linear-gradient(135deg,rgba(168,85,247,0.2),rgba(79,195,247,0.1))',border:'1px solid rgba(168,85,247,0.3)'}}>
          <span className="text-4xl">🎬</span>
        </div>

        {status === 'loading' && (
          <>
            <div className="h-6 w-6 rounded-full border-2 border-[#A855F7] border-t-transparent animate-spin"/>
            <p className="font-[Hind+Siliguri] text-white/60 text-[13px]">লোড হচ্ছে...</p>
          </>
        )}
        ...
        {status === 'monetag' && (
          <>
            <div>
              <p className="font-[Hind+Siliguri] font-black text-white text-[18px] mb-1">প্রায় হয়ে গেছে!</p>
              <p className="font-[Hind+Siliguri] text-white/50 text-[12px]">কয়েক সেকেন্ড অপেক্ষা করুন</p>
            </div>
            <div ref={monetagRef}/>
            {/* Countdown */}
            <div className="h-16 w-16 rounded-full border-2 border-[#A855F7]/40 flex items-center justify-center">
              <span className="font-black text-[#A855F7] text-[24px]">{countdown}</span>
            </div>
            <button onClick={goToChannel}
              className="mt-2 font-[Hind+Siliguri] font-black text-white text-[13px] px-6 py-3 rounded-2xl active:scale-95 transition-transform"
...
const ExclusiveZone = memo(({onPress}:{onPress:()=>void}) => (
  <section className="px-3 mb-4">
    <motion.div whileTap={{scale:0.97}} onClick={onPress}
      ...
      style={{boxShadow:'0 8px 40px rgba(245,158,11,0.2), 0 2px 8px rgba(0,0,0,0.5)'}}>
      {/* Background */}
      <div className="absolute inset-0" style={{background:'linear-gradient(135deg,#1a0900 0%,#2d1400 40%,#1a0c00 100%)'}}/>
      {/* Glow blobs */}
      <div className="absolute -top-6 -right-6 h-28 w-28 rounded-full pointer-events-none" style={{background:'radial-gradient(circle,rgba(251,191,36,0.18) 0%,transparent 70%)'}}/>
      <div className="absolute -bottom-4 -left-4 h-20 w-20 rounded-full pointer-events-none" style={{background:'radial-gradient(circle,rgba(251,191,36,0.10) 0%,transparent 70%)'}}/>
      {/* Top shimmer line */}
      ...
      {/* Border */}
      <div className="absolute inset-0 rounded-3xl" style={{border:'1px solid rgba(251,191,36,0.18)'}}/>

      <div className="relative z-10 px-5 py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          {/* Crown icon — no VIP tag */}
          ...
          <div>
            <p className="font-[Hind+Siliguri] font-black text-white text-[18px] leading-tight">
              {config.exclusive.title}
            </p>
            <p className="text-[10px] text-amber-400/60 font-semibold mt-0.5 tracking-wide">
              {config.exclusive.subtitle}
            </p>
          </div>
        </div>
        <div className="flex-shrink-0">
          <div className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl font-black text-black text-[11px] leading-none"
            style={{background:'linear-gradient(135deg,#fbbf24,#d97706)',boxShadow:'0 4px 16px rgba(245,158,11,0.35)'}}>
...
const CineVault = () => {
  ...
  const [adTarget,        setAdTarget]        = useState<string|null>(null); // ad page target channel
  ...
  const loadTMDB = useCallback(async()=>{
    ...
    try {
      ...
      setTrending(res1);
      // Hero pool: trending + popular movies mixed, with backdrop
      const heroItems = res1.filter((m:TMDBContent)=>m.backdrop_path).slice(0,8);
      ...
      const [r2,r3,r4,r5,r6,r7,r8,r9] = await Promise.all([
        ...
        // 🎭 South Indian — Telugu + Tamil আলাদা query, merge করা হবে
      ...
      // South: Telugu + Tamil merge করা
      const southRaw = [...(r8.results||[])];
      try {
        const tamilRes = await fetch(`${TMDB_BASE}/discover/movie?api_key=${TMDB_KEY}&with_original_language=ta&sort_by=popularity.desc`).then(r=>r.json());
        southRaw.push(...(tamilRes.results||[]));
      } catch(e){}
      setSouthMovies(southRaw.filter((m:TMDBContent)=>m.poster_path).slice(0,15));
      setAdultSeries(r9.results||FALLBACK);
      ...
      if (ytKey && ytKey !== 'YOUR_YOUTUBE_API_KEY') {
        ...
        await Promise.all(config.youtube.channels.map(async(ch: any, chIdx: number) => {
          try {
            ...
            if (ytRes.items && Array.isArray(ytRes.items)) {
          ...
          } catch(e){ console.warn(`YouTube fetch failed for ${ch.name}:`, e); }
        }));
  ...
  // ── Hero banner auto-rotate every 4s ──
  useEffect(()=>{
  ...
  // ── Telegram open (with ad) ──
  const openChannel = useCallback((username:string)=>{
    if(!username) return;
    // 24hr cooldown — আগে দেখলে সরাসরি যাবে
    if (hasSeenAdRecently()) {
      const tg = (window as any).Telegram?.WebApp;
      const url = username.startsWith('http') ? username : `https://t.me/${username}`;
      if(tg) tg.openTelegramLink(url.startsWith('https://t.me') ? url : `https://t.me/${username}`);
      else window.open(url, '_blank');
      return;
    }
    // Ad page দেখাও
    setAdTarget(username);
  },[]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0E0E12] text-white overflow-hidden">
      ...
      <AnimatePresence>
        {adTarget && (
          <motion.div key="adpage" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <AdPage
              channelUsername={adTarget}
              onDone={()=>setAdTarget(null)}
            />
          </motion.div>
        )}
      </AnimatePresence>
              ...
              <section className="px-3 mb-4 mt-2">
                <div onClick={()=>heroContent&&handlePosterClick(heroContent,config.heroBanner)}
                  ...
                  {heroContent&&(heroContent.backdrop_path||heroContent.poster_path) ? (
                    <>
                      <LazyImg key={heroContent.id} src={`${IMG_W500}${heroContent.backdrop_path||heroContent.poster_path}`}
                          ...
                          <div className="flex gap-1 mb-1.5">
                            {heroPool.slice(0,8).map((_,i)=>(
                              <div key={i} className="rounded-full transition-all duration-300"
                                style={{width: i===heroIndex?'16px':'5px', height:'5px',
