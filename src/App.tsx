...
const resolveChannel = (item: TMDBContent): string => {
  ...
  // Hindi / South Indian
  if (['hi','ta','te','ml','kn'].includes(lang)) return config.channelPortal.hindi;
  // Bengali
...
const CHANNEL_CATEGORIES = [
  {id:'movies', name:'মুভি চ্যানেল',         link:config.channelPortal.movies,  badge:'HD·4K',      grad:'from-[#0F172A] to-[#1E293B]', icon:<Film size={18}/>},
  {id:'series', name:'ওয়েব সিরিজ চ্যানেল',  link:config.channelPortal.series,  badge:'সব সিজন',   grad:'from-[#1E1B4B] to-[#312E81]', icon:<Library size={18}/>},
  {id:'anime',  name:'এনিমি দুনিয়া চ্যানেল',link:config.channelPortal.anime,   badge:'লেটেস্ট',   grad:'from-[#450A0A] to-[#7F1D1D]', icon:<Flame size={17}/>},
  {id:'kdrama', name:'কে-ড্রামা লাভার্স',    link:config.channelPortal.kdrama,  badge:'হিন্দি ডাব',grad:'from-[#064E3B] to-[#065F46]', icon:<Heart size={17}/>},
  {id:'hindi',  name:'হিন্দি ডাবড চ্যানেল', link:config.channelPortal.hindi,   badge:'সব মুভি',   grad:'from-[#312E81] to-[#1E1B4B]', icon:<Languages size={17}/>},
  {id:'bangla', name:'বাংলা ডাবড চ্যানেল',  link:config.channelPortal.bangla,  badge:'এক্সক্লুসিভ',grad:'from-[#134E4A] to-[#0D9488]', icon:<Clapperboard size={17}/>},
];
...
const PosterCard = memo(({movie,onClick,size='row'}:{
  ...
  const imgSrc = movie.poster_path ? `${size==='grid'?IMG_W185:IMG_W342}${movie.poster_path}` : '';
    ...
    <div className="grid grid-cols-2 gap-2">
      {CHANNEL_CATEGORIES.map(cat=>(
        <motion.div key={cat.id} whileTap={{scale:0.95}}
          ...
          className={`relative h-[66px] rounded-2xl bg-gradient-to-br ${cat.grad} px-3 flex items-center gap-2.5 border border-white/8 cursor-pointer overflow-hidden will-change-transform`}>
          ...
          <div className="flex-1 min-w-0">
            <p className="font-[Hind+Siliguri] font-black text-white text-[11px] leading-tight truncate">{cat.name}</p>
            <span className="text-[7px] text-white/40 font-bold uppercase tracking-wider">{cat.badge}</span>
          </div>
        </motion.div>
      ))}
    </div>
            ...
            <motion.div key="hub" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              ...
              <section className="px-3">
                <div className="relative rounded-2xl overflow-hidden"
                  style={{background:'linear-gradient(135deg,rgba(79,195,247,0.08) 0%,rgba(79,195,247,0.03) 100%)',border:'1px solid rgba(79,195,247,0.15)'}}>
                  <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#4FC3F7]/40 to-transparent"/>
                  <div className="px-4 py-3 flex items-start gap-3">
                    <div className="flex-shrink-0 mt-0.5">
                      <div className="h-7 w-7 rounded-full flex items-center justify-center" style={{background:'rgba(79,195,247,0.12)',border:'1px solid rgba(79,195,247,0.25)'}}>
                        <span className="text-[13px]">📢</span>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-[Hind+Siliguri] font-black text-[11px] text-[#4FC3F7] tracking-wide mb-0.5">নোটিশ</p>
                      <p className="font-[Hind+Siliguri] text-[11.5px] text-white/75 leading-relaxed">
