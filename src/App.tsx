...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────
// Silent Ad Runner — no UI, background only
// ─────────────────────────────────────────────
...
const runAd = async (onDone: () => void) => {
  // 24hr cooldown — আগে দেখলে সরাসরি
  if (hasSeenAdRecently()) { onDone(); return; }

  // ── Adsgram চেষ্টা ──
  try {
    await loadScript('https://sad.adsgram.ai/js/sad.min.js');
    const controller = await (window as any).Adsgram?.init({ blockId: ADSGRAM_BLOCK_ID });
    await controller.show();
    markAdShown();
    onDone();
    return;
  } catch (_) { /* Adsgram fail → Monetag */ }

  // ── Monetag fallback ──
  try {
    await loadScript('//libtl.com/sdk.js');
    const sdk = (window as any).show_9239674;
    if (sdk) {
      await new Promise<void>((res) => {
        sdk(MONETAG_ZONE_ID, { onComplete: () => res(), onError: () => res() });
        // timeout: 6s পরেও শেষ না হলে চলে যাবে
        setTimeout(res, 6000);
      });
    }
  } catch (_) {}
...
const CineVault = () => {
  ...
  // ── Telegram open (silent ad then channel) ──
